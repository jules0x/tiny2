...
const project = 'tiny', // Your project name
	PATHS = {
		srcDir: './src',
		distDir: '.' // The path of the dist directory, currently the theme root
	};
...
const path = '../../../_npm_environment.json',
	defaultConfig = {
		open: 'external',
		host: project + '.localhost', // this can be anything at .dev, or localhost, or...
		proxy: project + '.localhost', // this needs to be your project (localhost, .dev, vagrant domain et al)
		watchTask: true,
		injectChanges: true
	};
...
gulp.task('html', function() {
	return gulp.src(['templates/Page.ss']).pipe(browserSync.stream());
});

gulp.task('build', cb => {
	runSequence(['clean'], ['pure', 'json'], ['make-css', 'cms-css', 'make-js'], cb);
});
...
gulp.watch('templates/**/*.ss', ['html']); // Watch for template changes so we can stream to the browser
});
